...
int ringBufferPush(char *data, int len)
{
    ...
    {
        printf("ring buffer space is not enough\n");
...
int ringBufferPop(Nalu_ST *pstNalu)
{
    ...
    if (!(s_stRingbuffer.data[i % capacity] == 0x00 
        && s_stRingbuffer.data[(i + 1) % capacity] == 0x00 
        && s_stRingbuffer.data[(i + 2) % capacity] == 0x00 
        && s_stRingbuffer.data[(i + 3) % capacity] == 0x01))
    {
        ...
        printf("found the first 00 00 00 01 mark, throw away %d bytes\n", i);
    }


    i = (s_stRingbuffer.front + 4) % capacity; //先偏移4个标记位
    startIdx = i;
    int cnt = 0;
    ret = -2;
    while (i != s_stRingbuffer.rear)
    {
        ...
        && s_stRingbuffer.data[(i + 3) % capacity] == 0x01)
        {
            ...
            {
                printf("the nalu size is more than %d coredump will happen\n", ONE_NALU_MAX_SIZE);
                return -1;
            }
            ...
            if (startIdx + pstNalu->size > s_stRingbuffer.capacity) //分两次拷贝
            {
                copyLen = s_stRingbuffer.capacity - startIdx;
                memcpy(&pstNalu->frameData[4], &s_stRingbuffer.data[startIdx], copyLen);
                memcpy(&pstNalu->frameData[copyLen + 4], &s_stRingbuffer.data[0], pstNalu->size - copyLen);
            }
            else//一次拷贝完
            {
                memcpy(&pstNalu->frameData[4], &s_stRingbuffer.data[startIdx], pstNalu->size);
            }
